    ...
    renderTable() {
        ...
        columns.push(
            <tr key="0c">
                ...
                {this.state.values[0].map((v, i) => (<th className="bg-light" key={`-${i}`}>B<sub>{i + 1}</sub></th>))}
                <th className="bg-light">min(A<sub>i</sub>)</th>
            </tr>
        );

        let maxs = [];
        for (let i = 0; i < this.state.height; i++) {
            let rows = [<th className="bg-light" key={`min-${i}`}>A<sub>{i + 1}</sub></th>];
                ...
                rows.push((
                    <td key={`${i}-${j}`} className={calcBg(i, j)}>
                        <input type="number" value={val || ""}
                               ...
                               onChange={this.handleInput(i, j)} key={`${i}-${j}`}/>
                    </td>));
            }
            rows.push(<td className="bg-light" key={`${i}-min`}>{min}</td>);
            columns.push((<tr key={i}>{rows}</tr>));
        }

        columns.push(
            <tr key="lc">
                <th className="bg-light">max(B<sub>i</sub>)</th>
                {maxs.map((m, j) => (<td className="bg-light" key={`${j}-`}>{m}</td>))}
            </tr>
        );
